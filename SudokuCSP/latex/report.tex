...
\hline
 &  &  &  & ??? & $*$ \\ % training_rv=597.091732188 9m57s
...
Another interesting result, which can be seen in Tables~\ref{tab:heur_results} and \ref{tab:hs+heur_results}, is that the heuristics are all counterproductive. Either they are simply not effective or our implementation is not efficient enough.

\section{Conclusion}

We have written a Sudoku solver that treats Sudoku puzzles as CSPs and increased its performance by applying established CSP and Sudoku solving techniques. Unfortunately, our own heuristics had no positive effect.
